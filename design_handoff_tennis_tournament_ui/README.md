...
Tennis365風のテニス大会情報サービスの **スマホ向け（iPhone 375px幅）UIデザイン一式**。全国のテニス大会を横断検索・詳細確認・レポート閲覧できる、および提携スクールのコーチ求人を掲載する5画面構成のモバイルWeb。参考: https://tennis365.net/
...
1. **TOP** — 大会検索トップ
2. **一覧** — 検索結果ページ（Tennis365主催 / スクール主催が主催列で識別可能）
3. **詳細** — 大会詳細
...
**Purpose**: ユーザーが条件を絞って大会を探し始める入口。Tennis365主催大会 / スクール主催大会 の2枠を並列で見せる。

**Layout**:
- 縦1カラム、全体幅 375px 固定（コンテンツ幅）
- スクロールセクション: ①グローバルヘッダー → ②ヒーロー（検索ボックス埋め込み） → ③条件アコーディオン群 → ④月ナビ → ⑤Tennis365主催セクション + ページネーション → ⑥スクール主催セクション + ページネーション → ⑦スクール向けCTA → ⑧フッター
...
- **検索結果カード** (`ResultCard`, 画像なし・コンパクト)
  ...
  - 左端に3pxの borderLeft ストライプ: Tennis365主催 = `#6C9E1D`, スクール主催 = `#C89B3C`
...
**Purpose**: フィルタ済み検索結果を Tennis365主催 / スクール主催の識別を残したまま統合リストで見せる。

**Layout差分（TOPと共通のヘッダー/条件/月ナビを踏襲）**:
- ヘッダーに `showBack` を渡し、戻る矢印表示
- パンくず (`TsCrumb`): `HOME › 大会検索 › 一覧`
- 条件アコーディオンは同じ仕様（全て閉じ、選択数バッジで状態表示）
- 検索結果ヘッダ: "142件の大会" (数字は Manrope 800 18px) + ソート切替 (`開催日順` / `受付順`)
- **統合検索結果カード** (`ResultRow`):
  - 主催列を上部に横バーとして表示: `Tennis365 主催` = brand tint 背景 + `#4d7615` テキスト / `スクール主催` = accent tint 背景 + `#7a5a15` テキスト（9.5px 700 letter-spacing 0.08em）
  - 左端 3px の縦バーで主催種別を再度カラー識別（brand green / accent gold）
...
**Layout**:
- パンくず → ステータスバー (`Tennis365 主催` + `キャンセル待ち 0組` バッジ) → タイトル + サブタイトル → メイン画像+サムネイル4枚 → Quick Facts (`FactRow`, ダーク背景に2×2アイコングリッド) → **4タブ** (`概要 / 大会情報 / 会場 / その他`, sticky) → タブ内容 → この主催者の予定大会 → 関連リンクタイル (2×2) → フッター → **下部固定申込CTA**
...
- 申し込み方法セクション + メール `k.sawada@tennis365.net` 表示
...
- **スクールヘッダー**
  ...
  - Eyebrow "SCHOOL"、名前 "Tennis365 大会運営部" 15px 800、メタ情報
...
- **ハイライトレポート** (`FeatureReport`):
  - `border-left: 3px solid #6C9E1D`, 上部に170pxの画像 + 左上 `Tennis365 主催` の代わりに `ink` バッジ "最新レポート"
...
- 検索結果はサーバサイドフィルタ推奨（`GET /api/tournaments?month=&prefs=&cats=&types=&levels=&host_type=&page=&sort=`）
- レスポンス形状（1件）:
  ```json
  {
    "id": "string",
