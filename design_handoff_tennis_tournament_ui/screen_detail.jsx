  ...
  return (
    <div className="ts-screen ts-tap" style={{ minHeight: '100%', paddingBottom: 90 }}>
      ...
      <div style={{ display: 'flex', background: T.paper, borderBottom: `1px solid ${T.line}`, position: 'sticky', top: 0, zIndex: 5 }}>
        {['概要', '大会情報', '会場', 'その他'].map((t, i) => (
          <button key={t} onClick={() => setTabIdx(i)} style={{
            flex: 1, padding: '12px 4px', border: 'none', background: 'transparent',
            fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
            color: tabIdx === i ? T.ink : T.muted,
            borderBottom: tabIdx === i ? `2px solid ${T.brand}` : '2px solid transparent',
          }}>{t}</button>
        ))}
      </div>

      {tabIdx === 0 && <TabOverview />}
      {tabIdx === 1 && <TabInfo />}
      {tabIdx === 2 && <TabVenue />}
      {tabIdx === 3 && <TabOther />}
      ...
      <div style={{
        ...
        <button style={{
          width: 44, height: 44, border: `1px solid ${T.line}`, background: '#fff',
          borderRadius: 4, display: 'grid', placeItems: 'center', cursor: 'pointer', flexShrink: 0,
        }}>
          <svg width="18" height="18" viewBox="0 0 20 20"><path d="M10 17s-6-4-6-9a4 4 0 0 1 6-3.5A4 4 0 0 1 16 8c0 5-6 9-6 9z" stroke={T.ink} strokeWidth="1.5" fill="none"/></svg>
        </button>
...
function FactRow({ icon, label, value }) {
  const paths = {
    date: <><rect x="2" y="4" width="12" height="10" rx="1" stroke={T.brand} strokeWidth="1.4" fill="none"/><path d="M2 7h12M5 2v3M11 2v3" stroke={T.brand} strokeWidth="1.4" strokeLinecap="round"/></>,
    time: <><circle cx="8" cy="8" r="6" stroke={T.brand} strokeWidth="1.4" fill="none"/><path d="M8 4v4l3 2" stroke={T.brand} strokeWidth="1.4" fill="none" strokeLinecap="round"/></>,
    pin: <><path d="M8 1c-2.8 0-5 2.2-5 5 0 4 5 9 5 9s5-5 5-9c0-2.8-2.2-5-5-5z" stroke={T.brand} strokeWidth="1.4" fill="none"/><circle cx="8" cy="6" r="1.5" stroke={T.brand} strokeWidth="1.4" fill="none"/></>,
    yen: <><circle cx="8" cy="8" r="6" stroke={T.brand} strokeWidth="1.4" fill="none"/><path d="M5 5l3 3.5L11 5M5 9h6M5 11h6M8 8.5V13" stroke={T.brand} strokeWidth="1.2" fill="none" strokeLinecap="round"/></>,
  }[icon];
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
      <svg width="18" height="18" viewBox="0 0 16 16" style={{ marginTop: 1, flexShrink: 0 }}>{paths}</svg>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 9.5, color: '#c4d0c1', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 2 }}>{label}</div>
        <div style={{ fontSize: 12.5, fontWeight: 700, color: '#fff', letterSpacing: '0.01em' }}>{value}</div>
      </div>
    </div>
  );
}
...
function TabOverview() {
  return (
    <div style={{ background: T.paper, padding: '16px' }}>
      ...
      <SubHead style={{ marginTop: 20 }}>参加資格</SubHead>
      <ul style={ulStyle}>
        <li>チームのメンバー全員が初級〜中級程度であること</li>
      </ul>

      <SubHead style={{ marginTop: 20 }}>チーム構成</SubHead>
      <p style={pStyle}>
        男子2〜3名、女子2〜3名　計4〜6名<br/>
        （男子D、女子D、ミックスDの3本での対戦）
      </p>

      <SubHead style={{ marginTop: 20 }}>時間</SubHead>
      <p style={pStyle}>
        9時試合開始（8時50分頃より開会式を行いますので、それまでに受付を完了してください）<br/>
        17時終了予定<br/>
        （時間が余った場合、希望チームには練習試合を組みます）
      </p>

      {/* コンセプトカード */}
      <div style={{
        marginTop: 20, padding: '14px 14px', background: T.brandTint,
      ...
      <div style={{ marginTop: 16, padding: '12px 14px', background: '#fdf6e6', border: `1px solid ${T.accentTint}`, borderRadius: 3 }}>
...
function TabInfo() {
  const rows = [
    ...
    ['賞品', <>Tecnifibre(テク二ファイバー) ボール缶を全チームにプレゼント<br/><span style={{ color: T.muted, fontSize: 11 }}>何缶もらえるかは結果次第！</span></>],
  ];
  return (
...
const ulStyle = { margin: 0, paddingLeft: 20, fontSize: 13, color: T.ink, lineHeight: 1.75 };
