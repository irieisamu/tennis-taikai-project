...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    // component (Chrome/Safari). Trackpad scrolls send small/fractional
    // deltas — but macOS accelerates fast flicks and momentum ticks into
    // large integers with deltaX 0, per-event indistinguishable from wheel
    // notches. So classify per STREAM (events <180ms apart), deciding on
    // the first event only: trackpad gestures always ramp up from small
    // deltas, and re-deciding per event is exactly the bug where a
    // two-finger pan lurched into zoom once momentum crossed the integer
    // threshold. ctrlKey is set by the browser for trackpad pinch.
    let streamUntil = 0;
    let streamIsWheel = false;
    const isMouseWheel = (e) => {
      const inStream = e.timeStamp < streamUntil;
      streamUntil = e.timeStamp + 180;
      if (!inStream) {
        streamIsWheel =
          e.deltaMode !== 0 ||
          (e.deltaX === 0 && Number.isInteger(e.deltaY) && Math.abs(e.deltaY) >= 40);
      }
      return streamIsWheel;
    };

    const onWheel = (e) => {
      e.preventDefault();
      if (isGesturing) return; // Safari: gesture* owns the pinch — discard concurrent wheels
      const wheelish = isMouseWheel(e); // stateful — call exactly once per event
      if ((e.ctrlKey || e.metaKey) && !wheelish) {
