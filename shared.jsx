...
// ───────── 「条件から探す」開閉トグル（この行だけが開閉を持つ）
function TsFilterToggle({ open, onToggle, count, onClear }) {
  return (
    <div style={{ borderBottom: open ? `1px solid ${T.lineSoft}` : 'none' }}>
      <button
        onClick={onToggle}
        style={{
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: T.ink }}>条件から探す</span>
          {count != null && <TsBadge variant="brand" size="sm">{count}</TsBadge>}
        </span>
...
// ───────── フィルター項目（常時展開・個別には閉じない）
function TsFilterField({ label, count, children }) {
  return (
    <div style={{ padding: '10px 16px 16px', borderBottom: `1px solid ${T.lineSoft}` }}>
...
Object.assign(window, {
  T, TsHeader, TsCrumb, TsFilterToggle, TsFilterField, TsCheck, TsBadge, TsImgSlot,
